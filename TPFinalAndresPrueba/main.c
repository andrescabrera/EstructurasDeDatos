...
int altaBarrio (nBarrio **barrio, char objetivo[]);
        ...
        else
        {
            do
            {
                ...
                case 1:
                    ...
                    scanf(" %[^\n]", objetivo);
                    int op = altaBarrio(&barrios, objetivo);
                    if(op == 1)
                        mostrarxBarrio(barrios);
                    else
                ...
                case 4:
                    ...
                    if (opcion) mostrarOrdenada(barrios, "precio");
                    else
...
void cargar(nBarrio *barrios,  FILE *i, nBarrio **fin)
{
    ...
    while (!feof(i))
    {
        ...
        *fin = barrios;
        ...
        if(feof(i) && unentero == 0)
        {
            strcpy(barrios->nombarrio, unstring);
            barrios->sigBar = NULL; //cierro la lista de barrios
            barrios->baInm = NULL;
        }
    }
}
...
void mostrarOrdenada(nBarrio *barrios, char sortorder[40])
{
    ...
    while (b != NULL) //copio la lista
    {
        ...
        if (i != NULL)
        {
            while (i != NULL)
            {
                ...
                n->cantamb = i->cantamb;
                strcpy(n->dir, i->dir);
                //if(b->sigBar != NULL && i->sigInm != NULL)
    ...
    while (i->sigInm != NULL)
    {
        ...
        while (j->sigInm != NULL)
        {
            ...
            else if (j->precio < i->precio) op = 1;
            if (op)
            {
    ...
    while(nueva->sigInm != NULL)
    {
        printf("\n\tDireccion: %s\n\tAmbientes %d, Precio: %.2f\n", nueva->dir, nueva->cantamb, nueva->precio);
        nueva = nueva->sigInm;
    }
    system("pause");
}
...
{
    nInm *aux = NULL;
    if (b->baInm != NULL) //si tiene al menos un inmueble, sino paso de barrio
    {
    ...
    else if(b->sigBar != NULL)
        return modificarInmueble(b->sigBar, objetivo);
    else
        return 0;
}

int altaBarrio (nBarrio **barrio, char objetivo[40])
{
    nBarrio *nuevoregistro;
    nBarrio *existe = NULL;
    if(strcmp((*barrio)->nombarrio, objetivo) != 0)
    {
...
void cerrarSesion(nBarrio *barrio, FILE *i)
{
    ...
    if (barrio != NULL)
    {
        ...
        b = barrio->baInm;
        while (b != NULL) //imprimo sus inmuebles
        {
            fprintf(i, "\n%s\n%d %.2f", b->dir, b->cantamb, b->precio);
            b = b->sigInm;
        }
        if (barrio->sigBar != NULL) fprintf(i, "\n");
...
nInm *localizaInmueble (nInm *registro, char objetivo[])
{
    if (strcmp(registro->sigInm->dir,objetivo)==0)
        return registro;
    else if (registro->sigInm->sigInm!=NULL)
